...
export function CalendarView({ userData, onNavigate }: CalendarViewProps) {
  ...
  const upcomingSessions = sessions.filter(s => new Date(s.date) > new Date('2024-03-15'));
      ...
      <div className="bg-white border-b border-gray-200 px-8 py-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Manage your learning and teaching sessions</p>
          </div>
                ...
                {todaySessions.map((session) => (
                  <div key={session.id} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                    <div className="flex items-start justify-between mb-2">
                      <Badge variant={session.type === 'teaching' ? 'secondary' : 'default'} className="text-xs">
            ...
            {viewMode === 'week' ? (
              ...
              <div className="border rounded-lg overflow-hidden">
                ...
                <div className="grid grid-cols-8 border-b bg-gray-50">
                  ...
                  {weekDays.map((day, index) => (
                    <div key={day} className="p-3 text-center border-r last:border-r-0">
                      ...
                      <div className="text-xs text-gray-500 mt-1">{11 + index}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="divide-y">
                  {timeSlots.map((time) => (
                    <div key={time} className="grid grid-cols-8 hover:bg-gray-50 transition-colors">
                      <div className="p-3 border-r bg-gray-50 text-sm text-gray-600">
                        {time}
                      </div>
                      {weekDays.map((day, dayIndex) => (
                        <div key={`${day}-${time}`} className="p-2 border-r last:border-r-0 min-h-16 relative">
                          ...
                          {dayIndex === 0 && time === '10:00 AM' && (
                            <div className="absolute inset-1 bg-purple-100 border border-purple-300 rounded p-2 cursor-pointer hover:shadow-md transition-shadow">
                              <div className="text-xs font-medium text-purple-900 mb-1">JavaScript Fundamentals</div>
                              <div className="text-xs text-purple-700">Alex Johnson</div>
                            </div>
                          )}
                          {dayIndex === 0 && time === '2:00 PM' && (
                            <div className="absolute inset-1 bg-blue-100 border border-blue-300 rounded p-2 cursor-pointer hover:shadow-md transition-shadow">
                              <div className="text-xs font-medium text-blue-900 mb-1">Spanish Conversation</div>
                              <div className="text-xs text-blue-700">Carlos Rodriguez</div>
                            </div>
                          )}
                          {dayIndex === 1 && time === '11:00 AM' && (
                            <div className="absolute inset-1 bg-purple-100 border border-purple-300 rounded p-2 cursor-pointer hover:shadow-md transition-shadow">
                              <div className="text-xs font-medium text-purple-900 mb-1">React Components</div>
                              <div className="text-xs text-purple-700">Emma Thompson</div>
                            </div>
                          )}
                        </div>
                      ))}
              ...
              <div className="border rounded-lg overflow-hidden">
                <div className="grid grid-cols-7 border-b bg-gray-50">
                  {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map((day) => (
                    <div key={day} className="p-3 text-center border-r last:border-r-0 font-medium text-sm">
                      {day}
                    </div>
                  ))}
                </div>
                <div className="grid grid-cols-7">
                  {Array.from({ length: 35 }).map((_, index) => {
                    ...
                    const dateStr = isCurrentMonth ? `2024-03-${String(dayNum).padStart(2, '0')}` : '';
                    const daySessions = dateStr ? getSessionsForDate(dateStr) : [];
